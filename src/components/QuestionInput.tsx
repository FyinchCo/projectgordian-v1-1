...
export const QuestionInput = ({
  ...
  return <div className="space-y-8">
      ...
      <div className="text-center space-y-6">
        ...
        {customArchetypes && <p className="text-sm font-inter text-gordian-dark-brown bg-gordian-gold/20 inline-block px-3 py-1 rounded-full">
            Using {customArchetypes.length} custom archetypes from your configuration
          </p>}
        {enhancedMode && <p className="text-sm font-inter text-purple-800 bg-purple-100 inline-block px-3 py-1 rounded-full">
      ...
      <Card className="p-8 shadow-lg border-2 border-gordian-beige bg-white">
        <div className="space-y-6">
          <Textarea placeholder="Enter your deep, complex question here..." value={question} onChange={e => setQuestion(e.target.value)} className="min-h-[150px] text-lg font-inter border-0 shadow-none resize-none focus-visible:ring-0 p-0 placeholder:text-gordian-brown/50" />
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-4 border-t border-gordian-beige">
            <div className="flex flex-wrap items-center gap-4">
              ...
              {onToggleAssessment && <Button onClick={onToggleAssessment} variant="ghost" size="sm" className="flex items-center space-x-2 text-gordian-brown hover:text-gordian-dark-brown hover:bg-gordian-beige font-inter">
            ...
            <Button onClick={onStartGenius} disabled={!question.trim()} size="lg" className="bg-gordian-dark-brown text-gordian-cream hover:bg-gordian-brown flex items-center space-x-2 font-inter font-medium">
